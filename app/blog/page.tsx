        ...
        <div className='grid grid-cols-1 gap-8 divide-y sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4'>
          {posts.map((post: Post) => (
            <Link
              ...
              className='flex flex-row-reverse justify-between gap-y-1 pt-8 sm:flex-col 
              sm:rounded-md sm:border sm:pt-0 sm:shadow-lg sm:transition sm:duration-500 
              sm:ease-out sm:hover:scale-105 sm:hover:shadow-2xl'
            >
              <div className='relative h-44 w-48 overflow-hidden
               rounded-md sm:w-full sm:rounded-b-none sm:rounded-t-md'>
                <Image
                  src={post.image}
                  alt={post.title}
                  fill
